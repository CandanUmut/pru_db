...
use pru_core::{
    ...
    consts::SegmentKind,
    resolver_store::{ResolverStore, ResolveMode},
    manifest::Manifest,
    postings::{encode_sorted_u64, decode_sorted_u64, merge_sorted},
};
...
enum CliResolveMode { Union, Dedup, Intersect }

#[derive(Subcommand)]
enum Cmd {
    Init { #[arg(long)] dir: PathBuf },

    AddResolver {
        #[arg(long)] dir: PathBuf,
        #[arg(long)] key_hex: String,
        #[arg(long, num_args=1.., value_delimiter=',')] ids: Vec<u64>,
    },
    ...
    Verify { #[arg(long)] dir: PathBuf },

    Compact { #[arg(long)] dir: PathBuf },

    Promote { #[arg(long)] dir: PathBuf },

    Info { #[arg(long)] dir: PathBuf },
}

fn ensure_dir(p: &Path) -> Result<()> { std::fs::create_dir_all(p)?; Ok(()) }

fn now_ts() -> i64 { time::OffsetDateTime::now_utc().unix_timestamp() }
...
fn main() -> Result<()> {
    ...
    match cli.cmd {
        Cmd::Init{dir} => {
            ensure_dir(&dir)?;
            let m = Manifest::load(&dir)?; m.save_atomic(&dir)?;
        ...
        Cmd::AddResolver{dir, key_hex, ids} => {
            ...
            let mut lst = ids; lst.sort_unstable(); lst.dedup();
        ...
        Cmd::Resolve{dir, key_hex, and_key_hex, mode, set} => {
            let first = hex::decode(key_hex)?;
            let mut keys: Vec<Vec<u8>> = vec![first];
            for h in and_key_hex { keys.push(hex::decode(h)?); }
        ...
        Cmd::Verify{dir} => {
            ...
            let mut seg_ok=0usize; let mut seg_fail=0usize;
            let mut total=0usize; let mut bad_bounds=0usize; let mut bad_crc=0usize; let mut filter_miss=0usize;
            let mut total_slots: u64 = 0; let mut total_filled: u64 = 0;
            ...
            for s in &man.segments {
                ...
                match SegmentReader::open(&path) {
                    Ok(r) => {
                        ...
                        if r.kind == SegmentKind::Resolver {
                            ...
                            for e in r.iter() {
                                ...
                                if !r.verify_crc_at(e.off as usize, e.size as usize) { bad_crc += 1; }
                                if let Some(hit) = r.filter_contains_digest(e.hash) {
                                    if !hit { filter_miss += 1; }
                                }
                            }
                    ...
                    Err(e) => { eprintln!("verify: failed to open {}: {e}", path.display()); seg_fail += 1; }
                }
            }
            let lf = if total_slots>0 { (total_filled as f64)/(total_slots as f64) } else { 0.0 };
            println!("verify: segments ok={}, fail={}", seg_ok, seg_fail);
            println!("         entries={}  bad_bounds={}  bad_crc={}  filter_miss(XOR)={}", total, bad_bounds, bad_crc, filter_miss);
            println!("         load_factor(avg)≈{:.2} (filled={total_filled} / slots={total_slots})", lf);
        }
        Cmd::Compact{dir} => {
            let man = Manifest::load(&dir)?;
            ...
            for s in &man.segments {
                ...
                for e in r.iter() {
                    if let Some(val) = r.value_at(e.off as usize, e.size as usize) {
                        ...
                        lst.sort_unstable(); lst.dedup();
                        mp.entry(e.hash).and_modify(|acc| {
            ...
            if input_segments == 0 { return Err(anyhow!("no resolver segments to compact")); }
        ...
        Cmd::Promote{dir} => {
            ...
            if !man.archived_paths.is_empty() { println!("archived: {:?}", man.archived_paths); }
        }
        Cmd::Info{dir} => {
            let man = Manifest::load(&dir)?;
            ...
            for s in &man.segments {
                ...
                let mark = if act.iter().any(|p| *p == s.path) { '*' } else { ' ' };
                let mut extra = String::new();
                if let Ok(r) = SegmentReader::open(&full) {
                    if let Some((k, cap)) = r.index_meta() {
                        let filled = r.iter().count();
                        let lf = if cap>0 { (filled as f64)/(cap as f64) } else { 0.0 };
                        extra = format!(" entries={} cap={} load≈{:.2} kind={}", filled, cap, lf, k);
                    }
                }
                println!("{} {:?} {}{}", mark, s.kind, s.path.display(), if extra.is_empty(){String::new()} else {format!("  [{}]", extra)});
            }
        }
    }
    Ok(())
